...
def get_disk_analyzer() -> IDiskAnalyzer:
    if sys.platform.startswith("linux"):
        from os_optimizer.platform.linux.disk import LinuxDiskAnalyzer
...
def get_package_manager() -> IPackageManager:
    if sys.platform.startswith("linux"):
        from os_optimizer.platform.linux.packages import PacmanPackageManager
...
def get_health_checker() -> IHealthChecker:
    if sys.platform.startswith("linux"):
        from os_optimizer.platform.linux.health import LinuxHealthChecker
